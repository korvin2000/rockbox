                           ...
                           char *buf, int buflen)
{
    ...
    for(pass = 0; pass < 2 - track_first; pass++)
    {
        if (track_first || pass)
        {
            ...
            if (!found) {
                //splashf(HZ, "1: %s",path);
                nameonly = strrchr(trackname, '/') + 1;
//
//                size_t no_len = strlen(nameonly);
//                if (no_len > 128) {
//                    no_len = 128;
//                }
//                memcpy(name, nameonly, no_len);
//                name[no_len] = '\0';

                pathlen = snprintf(path, sizeof(path), "%sart/%s", dir, nameonly);
                //splashf(HZ*3, "3: %s",path);
                //fix_path_part(path, dirlen+strlen("art/"), strlen(name));
                found = try_exts(path, pathlen-3);
