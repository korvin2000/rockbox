...
CODECFLAGS := $(CFLAGS) $(RBCODEC_CFLAGS) -msoft-float -fstrict-aliasing \
...
$(CODECLIB) : CODECFLAGS += -O3
$(A52LIB) : CODECFLAGS += -Os
$(ALACLIB) : CODECFLAGS += -Os
$(ASAPLIB) : CODECFLAGS += -Os
$(ASFLIB) : CODECFLAGS += -O2
$(ATRACLIB) : CODECFLAGS += -Os
$(AYLIB) : CODECFLAGS += -O2
$(AYUMILIB) : CODECFLAGS += -O3
$(COOKLIB): CODECFLAGS += -Os
$(DEMACLIB) : CODECFLAGS += -Ofast
$(FAADLIB) : CODECFLAGS += -Ofast
$(FFMPEGFLACLIB) : CODECFLAGS += -Ofast
$(GBSLIB) : CODECFLAGS +=  -O3
$(HESLIB) : CODECFLAGS +=  -O3
$(KSSLIB) : CODECFLAGS +=  -O3
$(M4ALIB) : CODECFLAGS += -Ofast
$(MADLIB) : CODECFLAGS += -Ofast
$(MUSEPACKLIB) : CODECFLAGS += -Ofast
$(NSFLIB) : CODECFLAGS +=  -O2
$(OPUSLIB) : CODECFLAGS +=  -Ofast
$(PCMSLIB) : CODECFLAGS += -Os
$(RMLIB) : CODECFLAGS += -O3
$(SGCLIB) : CODECFLAGS +=  -O2
$(SPCLIB) : CODECFLAGS +=  -Os
$(SPEEXLIB) : CODECFLAGS += -Os
$(TREMORLIB) : CODECFLAGS += -Ofast
$(TTALIB) : CODECFLAGS += -O2
$(VGMLIB) : CODECFLAGS +=  -O2
$(EMU2413LIB) : CODECFLAGS +=  -O3
$(WAVPACKLIB) : CODECFLAGS += -Ofast
$(WMALIB) : CODECFLAGS += -O3
$(WMAPROLIB) : CODECFLAGS += -O3
$(WMAVOICELIB) : CODECFLAGS += -O3
